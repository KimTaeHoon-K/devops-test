...
const getResourceList = (pageNumber: number, pageSize: number) => {
  getVolumeSnapshotList(pageNumber, pageSize, props.volumeId)
    .then((res) => {
      if (res.status === 200) {
...
const volumeSnapshotDeleteForm = ref<VolumeSnapshotDeleteType>({
  snapshotIds: [] as string[],
});
// 자원 삭제 : 볼륨 스냅샷 삭제
const deleteResource = (idList: string[]) => {
  volumeSnapshotDeleteForm.value.snapshotIds = idList;
  deleteVolumeSnapshot(volumeSnapshotDeleteForm.value)
    .then((res) => {
      openNotification(
        true,
