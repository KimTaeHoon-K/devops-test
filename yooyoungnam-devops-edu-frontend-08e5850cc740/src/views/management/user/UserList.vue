  ...
  <a-page-header class="page-header" :title="t(route.meta?.title)">
    <template #extra>
      <a-button
        size="middle"
        type="default"
        @click="goToPageByName(router, 'user create')"
      >
      ...
      <a-button
        size="middle"
        type="default"
        @click="openConfirmModal('status-change')"
    ...
    >
      ...
      <template #headerCell="{ column }">
        <template v-if="column.dataIndex === 'name'">{{
          t(column.title, { data: t('common.content.user') })
        }}</template>
        <template v-else>
          {{ t(column.title) }}
        </template>
      ...
      <template #bodyCell="{ column, text, record }">
        <template v-if="column.dataIndex === 'name'">
          <a
            @click="
              goToPageByName(router, 'user detail', {
                id: record.id,
              })
            "
            >{{ text ? text : '(No name)' }}</a
          ...
          <a-dropdown placement="bottomRight">
            <template #overlay>
              <a-menu>
                <a-menu-item
                  @click="
                    goToPageByName(router, 'user edit', {
                      id: record.id,
                    })
                  "
                  >{{ t('common.content.action.edit') }}
                </a-menu-item>
                <a-menu-item @click="openConfirmModal('status-change', record)"
                  >{{ t('common.content.action.status-change') }}
                </a-menu-item>
...
import { getUserList } from '@/axios/identity-service/UserApi';
...
const selectedData = ref<{ rows: UserType[]; keys: string[] }>({
  rows: [],
  keys: [],
});
...
const openConfirmModal = (action: string, user?: UserType) => {
  if (action === 'delete') {
  ...
  if (user && user.id) {
    selectedData.value = { keys: [], rows: [] };
    selectedData.value.keys.push(user.id);
    selectedData.value.rows.push(user);
  }
