  ...
  <a-page-header class="page-header" :title="t(route.meta?.title)">
    <template #extra>
      <a-button
        size="middle"
        type="default"
        @click="goToPageByName(router, 'project create')"
      >
        {{ t('common.content.action.create') }}
      </a-button>
      <!--      <a-button size="middle" type="default">-->
      <!--        {{ t('common.content.action.block') }}-->
      <!--      </a-button>-->
      <a-button
        size="middle"
        type="default"
    ...
    >
      ...
      <template #bodyCell="{ column, text, record }">
        <template v-if="column.dataIndex === 'name'">
          <a
            @click="
              goToPageByName(router, 'project detail', {
                id: record.id,
              })
            "
            >{{ text ? text : '(No name)' }}</a
        ...
        <template v-if="column.dataIndex === 'enabled'">
          <span>
            <a-tag :key="text" :color="text ? 'geekblue' : 'lightgray'">
          ...
          <a-dropdown placement="bottomRight">
            <template #overlay>
              <a-menu>
                <a-menu-item
                  @click="
                    goToPageByName(router, 'project edit', {
                      id: record.id,
                    })
                  "
                ...
                <a-menu-item>{{
                  t('common.content.action.quota-edit')
                }}</a-menu-item>
                <a-menu-item>{{
                  t('common.content.action.user-edit')
                }}</a-menu-item>
                <a-menu-item>{{
                  t('common.content.action.tag-edit')
                }}</a-menu-item>
                <a-menu-item>{{
                  t('common.content.action.block')
                }}</a-menu-item>
...
import {
  deleteProject,
  getProjectList,
} from '@/axios/identity-service/ProjectApi';
...
const selectedData = ref<{ rows: ProjectType[]; keys: string[] }>({
  rows: [],
  keys: [],
});
...
const getResourceList = (pageNumber: number, pageSize: number) => {
  getProjectList(pageNumber, pageSize)
    .then((res) => {
      if (res.status === 200) {
...
const openConfirmModal = (action: string, project?: ProjectType) => {
  if (action === 'delete') {
  ...
  if (project && project.id) {
    selectedData.value = { keys: [], rows: [] };
    selectedData.value.keys.push(project.id);
    selectedData.value.rows.push(project);
  }
...
// 자원 삭제 : 프로젝트 삭제
const projectDeleteForm = ref<ProjectDeleteType>({
  projectIds: [] as string[],
});
const deleteResource = (idList: string[]) => {
  projectDeleteForm.value.projectIds = idList;
  deleteProject(projectDeleteForm.value)
    .then((res) => {
      openNotification(
        true,
